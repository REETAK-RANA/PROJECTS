...
# HTML template (dashboard with charts)
html_template = '''
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
    <div class="container">
      ...
      <div class="dashboard">
        ...
        <div class="panel">
          <h2>Change in Cost</h2>
          <canvas id="costChangeChart" width="200" height="200"></canvas>
        </div>
        <div class="panel">
    ...
    <script>
      function renderCost() {
        fetch('/cost').then(res=>res.json()).then(data=>{
          new Chart(document.getElementById('costChart'), {
            type:'doughnut',
            data:{
              labels:['Electricity','Gas'],
              datasets:[{data:[data.electricity,data.gas],backgroundColor:['#2bdcff','#ffe100']}]
            },
            options:{plugins:{title:{display:true,text:'Total $'+data.total,font:{size:16}}},legend: {labels:{color:'#eee'}}}
          });
        });
      }

      function renderCostChange() {
        fetch('/cost_change').then(res=>res.json()).then(data=>{
          new Chart(document.getElementById('costChangeChart'), {
            type:'bar',
            data:{
              labels:['May','Jun'],
              datasets:[{label:'Cost', data:[data.previous,data.current], backgroundColor:['#2bdcff','#2bdcff']}]
            },
            options:{plugins:{title:{display:true,text:data.percent+'% Increase',color:'#f44336'}},scales:{y:{color:'#eee'}},legend: {labels:{color:'#eee'}}}
          });
        });
      }
...
@app.route('/cost')
def cost():
    electricity = sum([v["current_usage"] for k,v in appliances.items() if k!="heater"])
    gas = appliances["heater"]["current_usage"]
    total = electricity + gas
    if total == 0:
        electricity = random.randint(150,180); gas = random.randint(30,50); total = electricity + gas
    return jsonify({"electricity": electricity, "gas": gas, "total": total})

@app.route('/cost_change')
def cost_change():
    prev = random.randint(190,210)
    curr = random.randint(200,220)
    change = round(100*(curr - prev)/prev, 2)
    return jsonify({"previous": prev, "current": curr, "percent": change})
